...
import ScapeTeaBot.modules.sql.feds_sql as sql
from ScapeTeaBot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, WOLVES,
                          dispatcher)
from ScapeTeaBot.modules.disable import DisableAbleCommandHandler
from ScapeTeaBot.modules.helper_funcs.alternate import send_message
from ScapeTeaBot.modules.helper_funcs.chat_status import is_user_admin
from ScapeTeaBot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from ScapeTeaBot.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @ScapeTeaBotSupport if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @ScapeTeaBotSupport should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        fed_name = info['fname']
        #https://t.me/ScapeTeaBotSupport/41606 // https://t.me/ScapeTeaBotSupport/41619
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @ScapeTeaBotSupport."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @ScapeTeaBotSupport !"
            )
            return
